...
| 维度 | DeepSeek Harness（dsh） | LangChain | CrewAI | AutoGen |
|---|---|---|---|---|
| 哲学 | 一切皆插件 | 链式组合 | 角色式 agent | 多 agent 对话 |
| 核心容器 | Cordis（插件内核） | 无内核 | 无内核 | 无内核 |
| 可替换性 | 三角色（定义/实现/消费） | 组件 | 有限 | 有限 |
| Agent 循环 | 内置 react 循环 | 自定义链 | 内置 | 内置 |
| 上下文压缩 | 内置（阈值触发） | 外部 | 外部 | 外部 |
| 会话持久化 | Append-only 事件日志 + 投影 | 手动 | 有限 | 有限 |
| Token 计量 | 真实 provider 回传用量 | 估算 | 估算 | 估算 |
| 插件隔离 | 作用域层（per-agent） | 无 | 无 | 无 |
| 跨进程 RPC | 内置（Ralph/ACP） | 无 | 无 | 无 |
| 学习曲线 | 陡峭（全功能） | 高（大量抽象） | 中 | 中 |
| 最适合 | 生产级 agent 系统 | 快速原型 | 角色式任务 | 研究实验 |

### 为什么这个教程聚焦 dsh

dsh 在 agent 框架中独树一帜：它有一个**真正的插件内核**（Cordis），而不是一组组件库。这意味着每个能力——模型调用、工具执行、会话日志、压缩——都是可替换、可组合、可作用域隔离的插件。理解这套架构，你获得的心智模型不止适用于 dsh——VS Code、Kubernetes Controller、webpack 都用了同样的模式。

这个教程是目前唯一逐机制拆解 dsh 的资料，从最小 agent 循环到完整端到端系统。配套的 [pydsh](https://github.com/ChenYu1991ppak/Pydsh) 项目提供了同一架构的可运行 Python 实现。
